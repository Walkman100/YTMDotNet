            ...
            input.Select(obj => obj as Dictionary<string, object>).Select(
                dict => {
                    ...
                    if (AddToToastAction_Item.ContainsKey("notificationActionRenderer")) {
                        ...
                        return new RateResultAction() {
                            ClickTrackingParams = dict["clickTrackingParams"] as string,
                            AddToToastAction_Item_NotificationTextRenderer = new RateResultActionNotificationTextRenderer() {
                                SuccessResponseText_Runs = GetText((NotificationTextRenderer["successResponseText"] as Dictionary<string, object>)["runs"] as List<object>),
